...
class BpxSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _bpx_create_server(super(), *args, **kwargs)
...
if sys.platform == "win32":
    ...
    class BpxProactor(IocpProactor):
        ...
        async def _bpx_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._bpx_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _bpx_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._bpx_accept_loop(listener)
...
def set_bpx_policy(connection_limit: int) -> None:
